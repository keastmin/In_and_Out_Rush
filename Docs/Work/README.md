...
작업자는 다음 두 가지 Git 동기화만 담당한다.

1. 에이전트가 요청할 때 최신 변경을 Pull한다.
2. 에이전트가 만든 Active 문서를 Commit·Push하고 완료 사실을 알린다.

상대 작업자의 Active 예약을 별도로 승인하거나 모든 기능 문서를 읽을 필요는 없다.
...
Active 문서 작성 후에는 구현하지 않는다. 에이전트는 `CheckReservation`으로 그 문서만 변경됐는지와 새 원격 변경을 확인하고, 작업자에게 Active 문서 Commit·Push를 요청한다.

작업자가 Push 완료를 알리면 `VerifyReservation`으로 다음을 확인한다.
...
구현 중 예약 범위가 늘어나면 Active 문서를 갱신하고 작업자가 다시 Push한 것을 확인한다.
